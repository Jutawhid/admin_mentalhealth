...
const validationSchema = Yup.object().shape({
    ...
    title: Yup.string().matches(/^[a-zA-Z]/,"Title must be start with alphabet character").required('Title is required').min(2,'Title must be greater than or equal 2 character').max(50,'Title must be less than 50 character'),
    
})



export const EditExpertType: React.FC = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, { setStatus, setSubmitting }) => {
            ...
            expertTypeAPI.updateExpertType(details.id,values).then(
                (res: any) => {
    ...
    const getDetails = (id: number) => {
        ...
        expertTypeAPI.getDetails(id).then(
          (res: any) => {
    ...
    return (
        <>
            ...
            <PageTitle>Update Expert Topic</PageTitle>
            {/* card start */}
            { loading ? <Loading/> : (
