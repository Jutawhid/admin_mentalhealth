...
const validationSchema = Yup.object().shape({
    ...
    //     return value > '0';
    // }),
    program_date: Yup.string().required('Publish Date is required').test('publishDate', "Please select the current year or a year within next 5 year ", (value: any) => {
        //console.log(value);
        if (value) {
    ...
    }).test('publishDate', "Select current time or next upcoming time", (value: any) => {
    ...
    }).test('fileType', 'Unsupported file format ! only png , jpg and jpeg required', (value) => {
        console.log(value);
...
export const AddNewWorkshop: React.FC = () => {

    //const [workshopTypeError, setWorkshopTypeError] = useState<string>("");
    //const [workshopTypeValues,setWorkshopTypeValues] = useState<number[]>([]);
    const [buttonLoading, setbuttonLoading] = useState<boolean>(false);
    const [workshopTypeData, setWorkshopTypeData] = useState<any>([]);
    const [defaultWorkshopTypeData, setDefaultWorkshopTypeData] = useState<any>();
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, { setStatus, setSubmitting }) => {
            ...
            formData.append("program_date", dateAndTime)
            formData.append('image', values.image);


            workshopAPI.addWorkshop(formData as any).then(
    ...
    const handleReset = () => {
        formik.resetForm();
        setDefaultWorkshopTypeData(null)
    }
    ...
    const selectValueChange = (item: any) => {
        // console.log(item)
        const workshopTypeId = item.map((val: any) => val.value)
        // console.log(workshopTypeId)
        formik.setFieldValue('workshop_type_id', workshopTypeId);
        setDefaultWorkshopTypeData(item)
    }
    ...
    return (
        <>
            ...
            <PageTitle>Add Workshop</PageTitle>
                ...
                <div className="card-body">
                    <form onSubmit={formik.handleSubmit}>
                        ...
                        <div className="row mt-5">
                            ...
                            <div className="form-group col-md-6 form-m-top_upto_md">
                                <label htmlFor="image" className="required">Workshop Image</label>
                                <input
                                    type="file"
                                    autoComplete="off"
                                    id="image"
