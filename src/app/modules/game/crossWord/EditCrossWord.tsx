...
const validationSchema = Yup.object().shape({
    ...
    answer:Yup.string().matches(/^[a-zA-Z]+$/, "Question must be alphabet character ! No numeric and special character allowed").required('Answer is required').min(2,'Answer must be greater than 2 character').max(7,'Answer must be less than or equal 7 character'),
    
})



export const EditCrossWord:React.FC = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, { setStatus, setSubmitting }) => {
            ...
            crossWordAPI.updateCrossWord(details.id,values.topic_id,values.question,answer).then(
                    (res: any) => {
    ...
    return (
        <>
            ...
            <PageTitle>Update Cross Word</PageTitle>
            {/* card start */}
            { loading ? <Loading/> : (
