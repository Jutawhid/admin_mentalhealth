...
const validationSchema = Yup.object().shape({
    ...
    question: Yup.string().matches(/^[a-zA-Z]/, "Question must be start with alphabet character").required('Question is required').min(2, 'Question must be greater than 2 character').max(200, 'Question must be less than 200 character'),
...
export const EditMCQ: React.FC = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, { setStatus, setSubmitting }) => {
               ...
               mcqAPI.updateMcq(details.id,values, answer).then(
                    (res: any) => {
    ...
    const handleReset = () => {
        ...
        details.options.forEach((val: any, index: number) => {
            const key = `option_${index + 1}`

            setMCQOption((pre: any) => {
                return {
                    ...pre,
                    [key]: val
                }
        ...
        if( Object.keys(details).length !==0 && activeGameTopic.length !== 0 ){
            setDefaultGameTopic(activeGameTopic.find((val:any)=> val.value === details.topic_id))
            formik.setFieldValue('topic_id',details.topic_id);
    ...
    useEffect(() => {
        ...
        if (Object.keys(details).length !== 0) {
            ...
            initialValues.answer = details.answer.toString();
            console.log(initialValues.question)
            // const gameTopicValue = {
            //     value: details.topic_id,
            //     label: details.topic_title,
            // }
            // formik.setFieldValue('topic_id', gameTopicValue.value);
            // setDefaultGameTopic(gameTopicValue)
    ...
    return (
        <>
            ...
            <PageTitle>Update MCQ Question</PageTitle>
            {/* card start */}
            {loading ? <Loading /> : (
